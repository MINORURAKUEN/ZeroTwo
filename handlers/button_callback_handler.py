...
Soporte para: Compresión, Selección de Audio, Subtítulos y Subdrips.
"""
...
def register(app, user_states, work_dir):
    ...
    async def button_callback(client, callback_query: CallbackQuery):
        ...
        if data == "list_audio":
            ...
            btns = [[InlineKeyboardButton(a['label'], callback_data=f"set_a_{a['index']}")] for a in tracks['audio']]
        ...
        # 2. Listar Subtítulos (Detección de Subdrips incluida)
        if data == "list_sub":
            tracks = VideoProcessor.probe_media(video_path)
            btns = [[InlineKeyboardButton(s['label'], callback_data=f"set_s_{s['index']}")] for s in tracks['subtitle']]
            btns.append([InlineKeyboardButton("⬅️ Volver", callback_data="back_to_main")])
            await callback_query.edit_message_reply_markup(InlineKeyboardMarkup(btns))
            return
        ...
        if data.startswith("set_a_"):
            ...
            await callback_query.answer("🔊 Audio seleccionado")
            return

        # 4. Guardar selección de Subtítulo
        if data.startswith("set_s_"):
            state['selected_sub'] = data.split("_")[-1]
            await callback_query.answer("📝 Subtítulo/Subdrip seleccionado")
            return
        ...
        # 6. Ejecutar Quema de Subtítulos
        if data == "start_burn_process":
            ...
            await callback_query.message.edit_text("⏳ Quemando subtítulos con alta calidad (CRF 20)...")
            ...
            if success:
                await callback_query.message.reply_video(str(output_burn), caption="✅ Quema completada")
                output_burn.unlink()
            else:
                await callback_query.message.reply_text("❌ Error al procesar la quema.")
            return

        # --- SECCIÓN: COMPRESIÓN (Tu lógica original) ---
        ...
        if data.startswith('format_'):
            ...
            await callback_query.message.edit_text("Ahora selecciona la resolución:", reply_markup=keyboard)
            return
        ...
        if data.startswith('quality_'):
            ...
            await callback_query.message.edit_text(f"⚙️ Comprimiendo a {config['label']}...")
            ...
            if success:
                await callback_query.message.reply_document(str(output_path), caption=f"✅ Comprimido a {config['label']}")
                output_path.unlink()
