...
def _buscar_imagen_animeplanet(titulo_romaji: str, titulo_english: str) -> str | None:
    """
    Busca la imagen de portada en Anime-Planet scrapeando el og:image.
    Anime-Planet usa CDN propio (anime-planet.b-cdn.net) con imágenes de buena calidad.
    Intenta construir el slug desde el título y extraer og:image del HTML.
    """
    import urllib.parse
    import re as _re

    def _to_slug(titulo: str) -> str:
        """Convierte título a slug estilo Anime-Planet."""
    ...
    headers = [
        ...
        '-H', 'Referer: https://www.anime-planet.com/',
    ]

    for titulo in [titulo_english, titulo_romaji]:
        if not titulo:
            continue
        slug = _to_slug(titulo)
        if not slug:
            continue
        url = f'https://www.anime-planet.com/anime/{slug}'
        try:
            cmd = ['curl', '-s', '-L', url, '--max-time', '12'] + headers
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
            if result.returncode != 0 or not result.stdout:
                continue
            # Extraer og:image del HTML
            match = _re.search(r'<meta[^>]+property=["\']og:image["\'][^>]+content=["\'](https://[^"\']+)["\']', result.stdout)
            if not match:
                match = _re.search(r'<meta[^>]+content=["\'](https://[^"\']+)["\'][^>]+property=["\']og:image["\']', result.stdout)
            if match:
                img_url = match.group(1)
                # Verificar que sea imagen del anime (no logo del sitio)
                if 'anime-planet' in img_url or 'b-cdn.net' in img_url:
                    logger.info(f"  🟩 Anime-Planet imagen: {img_url[:80]}")
                    return img_url
        except Exception as e:
            logger.warning(f"  ⚠ Anime-Planet error para {slug}: {e}")
            continue
...
def register(app, user_states, work_dir):
    ...
    async def anime_command(client, message: Message):
        ...
        try:
            ...
            # Candidatos en orden de calidad:
            # 1. Anime-Planet (og:image, buena resolución)
            # 2. AniList por ID directo
            # 3. AniList URLs de la API
            image_candidates = []

            ap_url = _buscar_imagen_animeplanet(titulo, titulo_ingles)
            if ap_url:
                image_candidates.append(ap_url)

            if anime_id:
                image_candidates.append(
                    f"https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx{anime_id}.jpg"
                )
