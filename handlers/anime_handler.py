...
def _buscar_imagen_kitsu(titulo_romaji: str, titulo_english: str, titulo_nativo: str = '') -> list[str]:
    """
    Busca en Kitsu API y retorna lista de URLs de imagen en orden de calidad.
    Intenta múltiples títulos hasta encontrar resultados.
    """
    import urllib.parse
    urls = []

    # Orden: inglés → romaji → nativo
    candidatos = []
    for t in [titulo_english, titulo_romaji, titulo_nativo]:
        if t and t.strip() and t not in candidatos:
            candidatos.append(t.strip())

    for query in candidatos:
        q = urllib.parse.quote(query)
        data = _curl_get(f'https://kitsu.io/api/edge/anime?filter[text]={q}&page[limit]=3')
        if not data:
            continue
        items = data.get('data', [])
        if not items:
            continue
        # Tomar el primer resultado con posterImage válido
        for item in items:
            ...
            for key in ('original', 'large', 'medium', 'small'):
                ...
                if url and url not in found:
                    found.append(url)
            if found:
                logger.info(f"  🟦 Kitsu match: {query!r} → {len(found)} URLs")
    ...
    if not urls:
        logger.info(f"  🟥 Kitsu: sin resultados")
    return urls
...
def _obtener_mejor_imagen(
    ...
    1. Kitsu original (frecuentemente alta resolución)
    2. AniList extraLarge (construida manualmente desde URL large)
    3. AniList URLs restantes
    4. MyAnimeList large_image_url

    Para cada URL:
    - Verifica Content-Length con HEAD request
    - Si ≥ min_size → descarga y retorna inmediatamente
    - Si < min_size → guarda como fallback
    - Retorna la imagen más pesada encontrada
    """
    import re as _re

    # ── Construir extraLarge de AniList desde la URL large ───────────────
    # AniList large:    .../cover/large/bx12345-HASH.jpg
    # AniList extraLarge: .../cover/extraLarge/bx12345-HASH.jpg
    extra_large_urls = []
    for url in anilist_urls:
        if '/cover/large/' in url:
            xl = url.replace('/cover/large/', '/cover/extraLarge/')
            if xl not in extra_large_urls:
                extra_large_urls.append(xl)

    # ── Construir lista ordenada de candidatos ───────────────────────────
    candidates: list[str] = []

    # 1. Kitsu (mejor calidad potencial)
    kitsu_urls = _buscar_imagen_kitsu(titulo_romaji, titulo_english, titulo_nativo)
    candidates.extend(kitsu_urls)

    # 2. AniList extraLarge construida manualmente
    candidates.extend([u for u in extra_large_urls if u not in candidates])

    # 3. AniList URLs originales recibidas
    candidates.extend([u for u in anilist_urls if u and u not in candidates])

    # 4. MAL como último recurso externo
    ...
    async def anime_command(client, message: Message):
        ...
        try:
            ...
            if img_data:
                ...
                # Si imagen ≥ 500KB → enviar como documento para evitar
                # que Telegram la comprima. Caption solo en foto/doc pequeño.
                if img_size >= MIN_IMAGE_SIZE:
                    # Documento: no se comprime, llega en full quality
                    await message.reply_document(
                        document=str(temp_img),
                        caption=None,
                        force_document=True
                    )
